...
(function () {
  ...
  function buildPass() {
    ...
    return {
      ...
      npcs: [
        ...
        {
          // アルシオンの末裔。海底の碑文を読んで戻ると素性を明かし、仲間になる
  ...
  function buildValley() {
    ...
    g.rect(2, 3, 24, 16, '.');                     // 谷底
    g.rect(12, 1, 4, 20, ',');                     // 参道（広すぎると墓所に見えない）
    // 歴代の竜の墓標を左右に並べる
    [[4, 5], [7, 5], [10, 5], [17, 5], [20, 5], [23, 5],
     [4, 8], [7, 8], [20, 8], [23, 8],
     [4, 11], [7, 11], [10, 11], [17, 11], [20, 11], [23, 11],
     [4, 15], [7, 15], [20, 15], [23, 15]].forEach(function (p2) { g.set(p2[0], p2[1], 'x'); });
    // 骨と枯れ木を散らす
    [[5, 6], [9, 9], [18, 7], [22, 12], [6, 17], [21, 17], [9, 13], [18, 16]]
      .forEach(function (p2) { g.set(p2[0], p2[1], 'J'); });
    [[3, 7], [24, 6], [3, 17], [24, 16], [9, 3], [18, 3]]
      .forEach(function (p2) { g.set(p2[0], p2[1], 'T'); });
    // 岩
